...
export class MeihuaAgent implements SubAgent {
  ...
  readonly description = '基于梅花易数起卦（邵雍时间起卦法）与体用生克进行占测分析';
  ...
  /**
   * 根据起卦时间（birthInfo 用作占测时间）生成卦象文本
   */
  ...
  async analyze(input: SubAgentInput): Promise<SubAgentOutput> {
    ...
    // 构建 prompt
    const builder = promptBuilderFactory.getBuilder('meihua')!;
